/**
 * @desc 1、isStatic：检测数据是不是除了symbol外的原始数据
 */
...
/**
 * @desc 2、isPrimitive：检测数据是不是原始数据
 */
...
/**
 * @desc 3、isObject：判断数据是不是引用类型的数据 (例如： arrays, functions, objects, regexes, new Number(0),以及 new String(''))
 */
...
/**
 * @desc 4、isObjectLike：检查 value 是否是 类对象。 如果一个值是类对象，那么它不应该是 null，而且 typeof 后的结果是 "object"
 */
...
/**
 * @desc 5、getRawType：获取数据类型，返回结果为 Number、String、Object、Array等
 */
...
/**
 * @desc 6、isPlainObject：判断数据是不是Object类型的数据
 */
...
/**
 * @desc 10、isNative：判断 value 是不是浏览器内置函数
...
/**
 * @desc 11、isFunction：检查 value 是不是函数
 */
...
/**
 * @desc 12、isLength：检查 value 是否为有效的类数组长度
 */
...
/**
 * @desc 13、isArrayLike：检查 value 是否是类数组
...
/**
 * @desc 17、hyphenate：驼峰命名转横线命名：拆分字符串，使用 - 相连，并且转换为小写
 */
...
/**
 * @desc 20、Object.assign：对象属性复制，浅拷贝
 */
...
/**
 * @desc 23、getExplorerInfo：获取浏览器信息
 */
...
/**
 * @desc 24、isPCBroswer：检测是否为PC端浏览器模式
 */
...
/**
 * @desc 27、repeat：生成一个重复的字符串，有n个str组成，可修改为填充为数组等
 */
...
/**
 * @desc 29、dateStrForma：将指定字符串由一种时间格式转化为另一种
 * from的格式应对应str的位置
 */
...
/**
 * @desc 30、getPropByPath：根据字符串路径获取对象属性 : 'obj[0].count'
 */
...
/**
 * @desc 31、GetUrlParam：获取Url参数，返回一个对象
 */
...
/**
 * @desc 32、downloadFile：base64数据导出文件，文件下载
 */
...
/**
 * @desc 35、requestAnimationFrame：window动画
 */
...
/**
 * @desc 39、random：返回一个lower - upper之间的随机数
...
/**
 * @desc 40、Object.keys：返回一个由一个给定对象的自身可枚举属性组成的数组
 */
...
/**
 * @desc 41、Object.values：返回一个给定对象自身的所有可枚举属性值的数组
 */
...
/**
 * @desc 42、arr.fill：使用 value 值来填充 array，从start位置开始, 到end位置结束（但不包含end位置），返回原数组
 */
...
/**
 * @desc 43、arr.includes：用来判断一个数组是否包含一个指定的值，如果是返回 true，否则false，可指定开始查询的位置
 */
...
/**
 * @desc 44、arr.find：返回数组中通过测试（函数fn内判断）的第一个元素的值
 */
...
/**
 * @desc 45、arr.findIndex ：返回数组中通过测试（函数fn内判断）的第一个元素的下标
 */
...
/**
 * @desc 46、performance.timing：利用performance.timing进行性能分析
 */
